...
2. **Domain Names:** tu subdominio (ej. `cardionet.ejemplo.com`).
3. **Forward Hostname / IP:** IP del servidor donde corre Docker (ej. `203.0.113.10` o `localhost` si NPM está en el mismo servidor).
4. **Forward Port:** `3010`.
...
Si quieres exponer la API en otro host (ej. `api.cardionet.ejemplo.com`):
- Crea otro Proxy Host con **Forward Port** `8083`.
- En `.env` añade ese origen a `CORS_ORIGINS` (ej. `https://api.cardionet.ejemplo.com`).
